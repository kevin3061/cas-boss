...
import tv.acframework.oss.boss.util.DES;
...
import tv.acframework.web.springmvc.controller.BaseController;
...
public class UserMngController extends BaseController {
	...
	public String save(Model model, User user, String[] roleIds, HttpServletRequest request) {
		try {
			...
			user.setJoinip(ip);
			User suser = (User)request.getSession().getAttribute(CasFilter.SESSION_USER_KEY);
			user.setCreateTime(DateUtil.getTodayDate());
			user.setUpdateTime(DateUtil.getTodayDate());
			user.setLastOperatorId(suser.getId());
			user.setLastOperatorName(suser.getUsername());
	...
	public String update(Model model, User user, String[] roleIds, HttpServletRequest request) {
		try {
			User suser = (User)request.getSession().getAttribute(CasFilter.SESSION_USER_KEY);
			user.setUpdateTime(DateUtil.getTodayDate());
			user.setLastOperatorId(suser.getId());
