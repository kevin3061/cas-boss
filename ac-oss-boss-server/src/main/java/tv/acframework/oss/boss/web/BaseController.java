...
import tv.acframework.oss.boss.util.DateEditor;
...
import tv.acframework.web.freemarker.directive.FlashMessageDirective;
...
public class BaseController {
	...
	protected void addSuccessMessage(Model model, String content, String url) {
		if ( model != null) {
			Message message = new Message(Message.Type.success, content, url);
	...
	protected void addSuccessMessage(Model model, String url) {
		if ( model != null) {
			Message message = new Message(Message.Type.success, url);
			model.addAttribute(FlashMessageDirective.FLASH_MESSAGE_ATTRIBUTE_NAME, message);
		}
	}
	...
	protected void addErrorMessage(Model model, String content) {
		if ( model != null) {
			Message message = new Message(Message.Type.error, content, "");
			model.addAttribute(FlashMessageDirective.FLASH_MESSAGE_ATTRIBUTE_NAME, message);
		}
	}
